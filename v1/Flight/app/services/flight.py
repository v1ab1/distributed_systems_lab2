...
class FlightService:
    ...
    ) -> tuple[list[FlightResponse], int]:
        flights_db, total_elements = await self._flight_repository.get_all(
            page, size
        )
        flights = [
            FlightResponse.model_validate(flight) for flight in flights_db
        ]
