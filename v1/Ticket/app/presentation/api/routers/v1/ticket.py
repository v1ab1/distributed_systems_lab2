...
async def save_new_ticket(
    ...
    ticket_uid = await ticket_service.save_new_ticket(body)
    response.headers["Location"] = f"/api/v1/tickets/{ticket_uid}"
    return None
...
async def cancel_ticket_by_uid(
    ...
    await ticket_service.cancel_ticket(str(ticket_uid))
